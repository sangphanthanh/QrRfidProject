		...
		if (!user) {
			User.addUser(newUser, (err, user) => {
				if (err) {
					res.json({
						success: false,
						msg: config.ST_Code12
					});
				} else {
					res.json({
						success: true,
						msg: config.ST_Code13
					});
		...
		switch (reason) {
			...
			case reasons.MAX_ATTEMPTS:
				...
				let mailOptions = {
					...
					html: '<b>Someone try to access account: </b>'+ username + '<br />'
 ...
 * Get list user 
 */
router.get('/listuser', passport.authenticate('jwt', {
	session: false
}), (req, res, next) => {
 ...
 * Change Password
 */
router.put('/changepasswd', passport.authenticate('jwt', {
	session: false
}), (req, res, next) => {
...
router.put('/resetpasswd', passport.authenticate('jwt', {
	...
	User.changepasswd(username, config.defaultpassdwd, (err, user) => {
		if (err) throw err;
		if (user) {
			return res.json({
				success: true,
				msg: config.ST_Code16
			});
		}
	})
});
